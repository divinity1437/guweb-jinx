Vue.mixin({
    methods: {
        ...
        timeDifference: function(date1, date2) {
            if (typeof date1 == "object") { date1 = date1.getTime() / 1000; }
        ...
        timeSince: function(date) {
            if (typeof date == "object") { date = date.getTime() / 1000; }
            var now = new Date();
            ...
            if (interval == 1) { interval = Math.floor(seconds / 3600); return "about " + interval + " hours ago"; }
            interval = Math.floor(seconds / 3600);
            ...
            if (seconds < 0) { console.log(date, currentTime); }
        ...
        getScoreMods(m, plus = true) {
            ...
            if (!hasNightcore && (m & DoubleTime)) { r += 'DT'; }
            if (m & HardRock) { r += 'HR'; }
            if (m & Perfect) {
                r += 'PF';
                hasPF = true;
            ...
            if (m & Relax) { r += 'RX'; }
            if (m & HalfTime) { r += 'HT'; }
            if (m & Flashlight) { r += 'FL'; }
            if (m & Autoplay) { r += 'AP'; }
            if (m & SpunOut) { r += 'SO'; }
            if (m & Relax2) { r += 'AP'; }
            if (!hasPF && (m & SuddenDeath)) { r += 'SD'; }
            if (m & Key4) { r += '4K'; }
            if (m & Key5) { r += '5K'; }
            if (m & Key6) { r += '6K'; }
            if (m & Key7) { r += '7K'; }
